...
def create_comparison_table_multiple_model(names, in_dir, data_indices, image_names_path, out_path):
    if torch.backends.mps.is_available():
        device = torch.device("mps")
    elif torch.cuda.is_available():
        device = torch.device("cuda")
    ...
    models = []
    for name in names:
        model = torch.load(f"{in_dir}/{name}.pth", map_location=device).eval()
        models.append(model)
    cpu = torch.device('cpu')
    ...
    for i in range(len(names)):
        axs[0, 2+i].text(0.5, 0.5, names[i], ha='center', va='center', fontweight='bold')
    for ax in axs[0]:
        ax.axis('off')
